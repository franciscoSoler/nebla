...
ControladorVendedor::ControladorVendedor(long numVendedor)
{
    ...
    this->pedidos = Cola<pedido_t>(NOMBRE_ARCHIVO, LETRA_COLA_CLIENTES + (char) 1 + (char) numVendedor);
    this->pedidos.obtener();
    
    /* Información sobre las órdenes de producción y compra. */
    this->shmemNumeroOrdenCompra = MemoriaCompartida(NOMBRE_ARCHIVO, LETRA_SHMEM_NRO_OC, sizeof(int));
    this->numeroOrdenCompra = (int*) shmemNumeroOrdenCompra.obtener();
    this->shmemNumeroOrdenProduccion = MemoriaCompartida(NOMBRE_ARCHIVO, LETRA_SHMEM_NRO_OP, sizeof(int));
    ...
    this->mutexAlmacenTerminados = Semaforo(NOMBRE_ARCHIVO, LETRA_SEM_ALMACEN_TERMINADOS);
