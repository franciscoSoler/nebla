...
int main(int argc, char **argv)
{
	...
	while (deboSeguir) {
		...
		int dormir = rand() % 15 + 1;
		sprintf (buffer, "Puerta Salida: %d Durmiendo por %d segundos\n", nroPuerta, dormir);
		write (fileno(stdout),buffer, strlen(buffer));
		usleep(dormir*1000*1000);
		
		totalPuerta++;
		
		sprintf (buffer, "Puerta Salida: %d Se va la persona %d\n", nroPuerta, totalPuerta);
		write (fileno(stdout),buffer, strlen(buffer));
		...
		if (semaphoreOperation == -1) exit(-1);
		{
			...
			if ((*info).cantidad == 0) {
				sprintf (buffer, "Puerta Salida: %d El museo esta VACIO, la persona %d nunca ingreso\n", nroPuerta,totalPuerta);
				write (fileno(stdout),buffer, strlen(buffer));					
			}
			else {
				sprintf (buffer, "Puerta Salida: %d La persona %d sale del museo\n", nroPuerta,totalPuerta);
				write (fileno(stdout),buffer, strlen(buffer));
