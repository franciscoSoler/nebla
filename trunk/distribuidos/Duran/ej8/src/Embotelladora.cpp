...
int main(int argc, char **argv)
{
	...
	if (argc != 2) {
		strcpy (buffer, "Error Embotelladora: Cantidad de parametros invalida.\n");
	...
	sprintf(buffer, "Embotelladora: Iniciando\n");
	...
	try {
		...
		sprintf (buffer, "Embotelladora: %s\n", ex.getDescription());
		write (fileno(stderr),buffer, strlen(buffer));
		exit(-1);
	}
	...
	while (deboSeguir) {
		try {
			semEmbotelladora.wait();
			
			srand(time(NULL));
			int delay = (rand() % 3) + 1;
			sprintf(buffer, "Embotelladora: Embotellando durante %d\n", delay);
			write(fileno(stdout), buffer, strlen(buffer));
			sleep(delay);
	...
	sprintf (buffer, "Embotelladora: Terminando\n");
	write (fileno(stdout),buffer, strlen(buffer));	
