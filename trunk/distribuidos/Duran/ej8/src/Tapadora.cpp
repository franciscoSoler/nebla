...
int main(int argc, char **argv)
{
	...
	if (argc != 2) {
		strcpy (buffer, "Error Tapadora: Cantidad de parametros invalida.\n");
	...
	sprintf(buffer, "Tapadora: Iniciando\n");
	...
	while (deboSeguir) {
		try {
			...
			srand(time(NULL));
			int delay = (rand() % 3) + 1;
			sprintf(buffer, "Tapadora: Tapando durante %d\n", delay);
			write(fileno(stdout), buffer, strlen(buffer));
			sleep(delay);
	...
	sprintf (buffer, "Tapadora: Terminando\n");
	write (fileno(stdout),buffer, strlen(buffer));	
